...
class RpushCommandTest {
    ...
    private final RpushCommand command = new RpushCommand(store);

    @Test
    void rpushSingleValueToNewList() {
        var result = command.execute(new RespValue[]{
                new BulkString("RPUSH".getBytes()),
    ...
    void rpushMultipleValues() {
        ...
        assertArrayEquals("c".getBytes(), list.get(2));
    }

    @Test
    void rpushToExistingList() {
        store.rpush("mylist", "a".getBytes());
        var result = command.execute(new RespValue[]{
                new BulkString("RPUSH".getBytes()),
                new BulkString("mylist".getBytes()),
                new BulkString("b".getBytes())
